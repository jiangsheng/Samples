                ...
                if (doc != null)
                {
                    ...
                    if (doc.Title == string.Empty)
                    {
                        webBrowser1.DocumentText =
                            "<html>\r\n<head>\r\n<title>Lorem ipsum</title><meta http-equiv=\"X-UA-Compatible\" content=\"IE=7\">\r\n</head>\r\n" +
                    ...
                    else if (doc.Title == "Lorem ipsum")
                    {
                        ...
                        var doc4 = doc.DomDocument as IHTMLDocument4;
                        if (doc2 != null)
                        {
                            if (doc2.designMode != "on")
                            {
                                var body = doc.Body.DomElement as IHTMLBodyElement;
                                var div1 = doc.GetElementById("div1").DomElement as IHTMLElement;
                                var div3 = doc.GetElementById("div3").DomElement as IHTMLElement;
                                IHTMLEditServices editServices = (IHTMLEditServices)ComHelper.QueryService(doc.DomDocument, EDIT_SERVICES_SID, typeof(IHTMLEditServices).GUID);

                                IMarkupServices markupServices = (IMarkupServices)doc.DomDocument;
                                ...
                                divEnds[0].MoveAdjacentToElement(div1, _ELEMENT_ADJACENCY.ELEM_ADJ_AfterBegin);
                                divEnds[1].MoveAdjacentToElement(div1, _ELEMENT_ADJACENCY.ELEM_ADJ_BeforeEnd);
                                divEnds[2].MoveAdjacentToElement(div3, _ELEMENT_ADJACENCY.ELEM_ADJ_AfterBegin);
                                divEnds[3].MoveAdjacentToElement(div3, _ELEMENT_ADJACENCY.ELEM_ADJ_BeforeEnd);

                                IDisplayServices displayServices = (IDisplayServices)doc.DomDocument;
                                ...
                                IHighlightRenderingServices highlightRenderingServices = doc.DomDocument as IHighlightRenderingServices;
                                IHighlightSegment highlightSegment; ;
                                highlightRenderingServices.AddSegment(displayPointers[0], displayPointers[1], renderStyle, out highlightSegment);
                                highlightRenderingServices.AddSegment(displayPointers[2], displayPointers[3], renderStyle, out highlightSegment);
                                //can turn design mode on here and wait for the document to switch mode.                              
                            }
                            else
                            {
                                //design mode turned on, more editing features enabled
                            }
                        }
                    }
                }
